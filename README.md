...
Принципы работы алгоритма по основаниям 2, 3 и 5 ничем друг от друга не отличаются, просто происходит разделение текущего ряда на 2, 3 или 5 рядов, из которых получаются нужные компоненты, которые в свою очередь домножаются на комплексные единицы и суммируются для получения исходной компоненты.
